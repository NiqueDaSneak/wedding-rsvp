...
const SiteHeader: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const textParticleCount = 8000; // Particles for text
    const backgroundParticleCount = 1000; // Fewer background particles
    ...
    // Vertex shader for particles - with improved positioning and no mouse interaction
    ...
    // Fragment shader for particle appearance - enhanced for better text visibility
    ...
    // Function to sample points from text - improved for better definition
    function sampleTextPoints(
      ...
      const scale = 2; // Super-sampling for better detail
      ...
      // Set text properties for main fill
      const fontSize = Math.min(width / 11, 55) * scale; // Smaller font for better fit
      offscreenCtx.fillStyle = 'white';
      offscreenCtx.textAlign = 'center';
      offscreenCtx.textBaseline = 'middle';
      offscreenCtx.font = `italic 700 ${fontSize}px 'Playfair Display', serif`;

      // Draw text with more spacing
      offscreenCtx.fillText(
        'Dominique',
        (width * scale) / 2,
        (height * scale) / 2 - fontSize * 1.5, // Less vertical spacing
      );

      // Ampersand
      const ampFontSize = fontSize * 0.9;
      offscreenCtx.font = `italic 700 ${ampFontSize}px 'Playfair Display', serif`;
      offscreenCtx.fillText('&', (width * scale) / 2, (height * scale) / 2);
      ...
      // Draw the text with a slight stroke for edge detection
      edgeCtx.clearRect(0, 0, width * scale, height * scale);
      edgeCtx.strokeStyle = 'white';
      edgeCtx.lineWidth = Math.max(2, fontSize / 30) * scale;
      edgeCtx.textAlign = 'center';
      edgeCtx.textBaseline = 'middle';

      // First name - match the positioning from above
      edgeCtx.font = `900 ${fontSize}px 'Playfair Display', serif`;
      edgeCtx.strokeText(
        'Dominique',
        (width * scale) / 2,
        (height * scale) / 2 - fontSize * 1.8, // Match the increased spacing
      );

      // Ampersand
      edgeCtx.font = `900 ${ampFontSize}px 'Playfair Display', serif`;
      edgeCtx.strokeText('&', (width * scale) / 2, (height * scale) / 2);

      // Second name - match the positioning from above
      edgeCtx.font = `900 ${fontSize}px 'Playfair Display', serif`;
      edgeCtx.strokeText(
        'Sabigaynn',
        (width * scale) / 2,
      ...
      // Use a smaller step size for precise sampling
      const sampleStep = 2;
    ...
    function setupWebGL() {
      ...
      canvas.height = 350; // Increased from 300 to 350 for more vertical space
      gl.viewport(0, 0, canvas.width, canvas.height);

      // Sample text points
      textPoints = sampleTextPoints(canvas.width, canvas.height);

      // Add these variables to the color palettes for more distinction
      // Enhanced green color palette with more contrast
      const textFillColors = [
        [0.325, 0.459, 0.078, 0.95], // #537614 - dark green (more opaque)
        [0.635, 0.773, 0.388, 0.9], // #A2C563 - medium green
      ];
      ...
      const backgroundColors = [
        [0.82, 0.929, 0.62, 0.3], // #D1ED9E - light green (more transparent)
        [0.635, 0.773, 0.388, 0.2], // #A2C563 - medium green (very transparent)
      ];
      ...
      for (let i = 0; i < textParticlesToAssign; i++) {
        ...
        if (isEdgePoint) {
          // Edge particles are smaller but more opaque
          sizes[i] = 1.8 + Math.random() * 0.7;
          ...
          // Fill particles are slightly larger
  ...
  return (
    <div className="site-header" ref={containerRef}>
